...
func BaseAdd_avx2_Float16(dst []hwy.Float16, s []hwy.Float16) {
	...
	for ; i+lanes*4 <= n; i += lanes * 4 {
		...
		result2 := vd2.Add(vs2)
		result2.StoreSlice(unsafe.Slice((*uint16)(unsafe.Pointer(unsafe.SliceData(dst[i+16:]))), len(dst[i+16:])))
		vd3 := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&dst[i+24:][0]))
		vs3 := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&s[i+24:][0]))
		result3 := vd3.Add(vs3)
		result3.StoreSlice(unsafe.Slice((*uint16)(unsafe.Pointer(unsafe.SliceData(dst[i+24:]))), len(dst[i+24:])))
	}
	if i < n {
