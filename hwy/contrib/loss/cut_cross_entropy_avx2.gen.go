...
func BaseCutCrossEntropy_avx2(hiddenStates []float32, embeddings []float32, labels []int32, numPositions int, hiddenDim int, vocabSize int) float32 {
...
func BaseCutCrossEntropyGrad_avx2(hiddenStates []float32, embeddings []float32, labels []int32, gradOutput []float32, numPositions int, hiddenDim int, vocabSize int) {
	if numPositions == 0 || hiddenDim == 0 || vocabSize == 0 {
		return
	}
	validCount := 0
...
func BaseCutCrossEntropyWithLogits_avx2(hiddenStates []float32, embeddings []float32, labels []int32, perPositionLoss []float32, correctLogits []float32, numPositions int, hiddenDim int, vocabSize int) float32 {
	if numPositions == 0 || hiddenDim == 0 || vocabSize == 0 {
		return 0
	}
