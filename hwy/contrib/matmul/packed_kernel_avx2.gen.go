...
func BasePackedMicroKernel_avx2(packedA []float32, packedB []float32, c []float32, n int, ir int, jr int, kc int, mr int, nr int) {
	...
	for p := 0; p < kc; p++ {
		...
		vB0 := archsimd.LoadFloat32x8Slice(packedB[bIdx:])
		vB1 := archsimd.LoadFloat32x8Slice(packedB[bIdx+lanes:])
		bIdx += nr
	...
	cRow2 := (ir + 2) * n
	cRow3 := (ir + 3) * n
	vC := archsimd.LoadFloat32x8Slice(c[cRow0+jr:])
	vC = vC.Add(acc00)
	vC.StoreSlice(c[cRow0+jr:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow0+jr+lanes:])
	vC = vC.Add(acc01)
	vC.StoreSlice(c[cRow0+jr+lanes:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow1+jr:])
	vC = vC.Add(acc10)
	vC.StoreSlice(c[cRow1+jr:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow1+jr+lanes:])
	vC = vC.Add(acc11)
	vC.StoreSlice(c[cRow1+jr+lanes:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow2+jr:])
	vC = vC.Add(acc20)
	vC.StoreSlice(c[cRow2+jr:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow2+jr+lanes:])
	vC = vC.Add(acc21)
	vC.StoreSlice(c[cRow2+jr+lanes:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow3+jr:])
	vC = vC.Add(acc30)
	vC.StoreSlice(c[cRow3+jr:])
	vC = archsimd.LoadFloat32x8Slice(c[cRow3+jr+lanes:])
	vC = vC.Add(acc31)
...
func BasePackedMicroKernel_avx2_Float64(packedA []float64, packedB []float64, c []float64, n int, ir int, jr int, kc int, mr int, nr int) {
	...
	for p := 0; p < kc; p++ {
		...
		vB0 := archsimd.LoadFloat64x4Slice(packedB[bIdx:])
		vB1 := archsimd.LoadFloat64x4Slice(packedB[bIdx+lanes:])
		bIdx += nr
		acc00 = vA0.MulAdd(vB0, acc00)
		acc01 = vA0.MulAdd(vB1, acc01)
		acc10 = vA1.MulAdd(vB0, acc10)
	...
	cRow2 := (ir + 2) * n
	cRow3 := (ir + 3) * n
	vC := archsimd.LoadFloat64x4Slice(c[cRow0+jr:])
	vC = vC.Add(acc00)
	vC.StoreSlice(c[cRow0+jr:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow0+jr+lanes:])
	vC = vC.Add(acc01)
	vC.StoreSlice(c[cRow0+jr+lanes:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow1+jr:])
	vC = vC.Add(acc10)
	vC.StoreSlice(c[cRow1+jr:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow1+jr+lanes:])
	vC = vC.Add(acc11)
	vC.StoreSlice(c[cRow1+jr+lanes:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow2+jr:])
	vC = vC.Add(acc20)
	vC.StoreSlice(c[cRow2+jr:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow2+jr+lanes:])
	vC = vC.Add(acc21)
	vC.StoreSlice(c[cRow2+jr+lanes:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow3+jr:])
	vC = vC.Add(acc30)
	vC.StoreSlice(c[cRow3+jr:])
	vC = archsimd.LoadFloat64x4Slice(c[cRow3+jr+lanes:])
	vC = vC.Add(acc31)
	vC.StoreSlice(c[cRow3+jr+lanes:])
}
	...
	for r := 0; r < mr; r++ {
		...
		for col = 0; col+lanes <= nr; col += lanes {
			...
			for p := 0; p < kc; p++ {
				...
				vB := archsimd.LoadFloat32x8Slice(packedB[p*nr+col:])
				acc = vA.MulAdd(vB, acc)
			}
			vC := archsimd.LoadFloat32x8Slice(c[cRowStart+jr+col:])
			vC = vC.Add(acc)
	...
	for r := 0; r < mr; r++ {
		...
		for col = 0; col+lanes <= nr; col += lanes {
			...
			for p := 0; p < kc; p++ {
				...
				vB := archsimd.LoadFloat64x4Slice(packedB[p*nr+col:])
				acc = vA.MulAdd(vB, acc)
			}
			vC := archsimd.LoadFloat64x4Slice(c[cRowStart+jr+col:])
			vC = vC.Add(acc)
			vC.StoreSlice(c[cRowStart+jr+col:])
		}
		for ; col < nr; col++ {
