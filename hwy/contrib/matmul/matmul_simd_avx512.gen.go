...
func BaseMatMul_avx512(a []float32, b []float32, c []float32, m int, n int, k int) {
	...
	for i := range m {
		...
		for p := range k {
			...
			bRow := b[p*n : (p+1)*n]
			for j = 0; j+lanes <= n; j += lanes {
				vB := archsimd.LoadFloat32x16Slice(bRow[j:])
				vC := archsimd.LoadFloat32x16Slice(cRow[j:])
				vC = vA.MulAdd(vB, vC)
...
func BaseMatMul_avx512_Float64(a []float64, b []float64, c []float64, m int, n int, k int) {
	...
	for i := range m {
		...
		for p := range k {
			...
			bRow := b[p*n : (p+1)*n]
			for j = 0; j+lanes <= n; j += lanes {
				vB := archsimd.LoadFloat64x8Slice(bRow[j:])
				vC := archsimd.LoadFloat64x8Slice(cRow[j:])
				vC = vA.MulAdd(vB, vC)
				vC.StoreSlice(cRow[j:])
			}
			for ; j < n; j++ {
