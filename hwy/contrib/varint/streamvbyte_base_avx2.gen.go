...
func BaseDecodeStreamVByte32_avx2(control []byte, data []uint8, n int) []uint32 {
	...
	BaseDecodeStreamVByte32Into_avx2(control, data, result)
	return result
}

func BaseDecodeStreamVByte32Into_avx2(control []byte, data []uint8, dst []uint32) (decoded int, dataConsumed int) {
	...
	for _, ctrl := range control {
		...
		if dstPos+4 <= n && dataPos+16 <= len(data) {
			consumed := BaseDecodeStreamVByte32GroupSIMD_avx2(ctrl, data[dataPos:], dst[dstPos:])
			if consumed > 0 {
				dataPos += consumed
				dstPos += 4
				continue
...
func BaseDecodeStreamVByte32GroupSIMD_avx2(ctrl byte, data []uint8, dst []uint32) int {
