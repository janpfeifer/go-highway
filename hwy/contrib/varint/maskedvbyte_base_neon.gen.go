...
func BaseMaskedVByteDecodeBatch32_neon(src []byte, dst []uint32, n int) (decoded int, consumed int) {
	...
	for decoded+4 <= maxDecode && pos+16 <= len(src) {
		numDecoded, bytesUsed := BaseMaskedVByteDecodeGroup_neon(src[pos:], dst[decoded:])
		if numDecoded == 0 {
			break
		}
		decoded += numDecoded
...
func BaseMaskedVByteDecodeGroup_neon(src []byte, dst []uint32) (decoded int, consumed int) {
	...
	maskVec := asm.LoadUint8x16Slice(shuffleMask)
