...
func BaseDecodeStreamVByte32_neon(control []byte, data []uint8, n int) []uint32 {
	...
	BaseDecodeStreamVByte32Into_neon(control, data, result)
	return result
}

func BaseDecodeStreamVByte32Into_neon(control []byte, data []uint8, dst []uint32) (decoded int, dataConsumed int) {
	...
	for _, ctrl := range control {
		...
		if dstPos+4 <= n && dataPos+16 <= len(data) {
			consumed := BaseDecodeStreamVByte32GroupSIMD_neon(ctrl, data[dataPos:], dst[dstPos:])
			if consumed > 0 {
				dataPos += consumed
				dstPos += 4
				continue
...
func BaseDecodeStreamVByte32GroupSIMD_neon(ctrl byte, data []uint8, dst []uint32) int {
	...
	dataVec := asm.LoadUint8x16Slice(data[:16])
	maskSlice := streamVByte32ShuffleMasks[ctrl][:]
	maskVec := asm.LoadUint8x16Slice(maskSlice)
	shuffled := dataVec.TableLookupBytes(maskVec)
	var result [16]uint8
...
func BaseEncodeStreamVByte32_neon(values []uint32) (control []byte, data []byte) {
	if len(values) == 0 {
		return nil, nil
	}
	...
	for g := 0; g < numGroups; g++ {
		...
		if remaining >= 4 {
			ctrl, groupData := BaseEncodeStreamVByte32GroupSIMD_neon(values[baseIdx : baseIdx+4])
			control[g] = ctrl
...
func BaseEncodeStreamVByte32Into_neon(values []uint32, controlBuf []byte, dataBuf []byte) (control []byte, data []byte) {
	if len(values) == 0 {
		return nil, nil
	}
	numGroups := (len(values) + 3) / 4
	...
	for g := 0; g < numGroups; g++ {
		...
		if remaining >= 4 && dataPos+16 <= len(dataBuf) {
			ctrl, n := BaseEncodeStreamVByte32GroupSIMDInto_neon(values[baseIdx:baseIdx+4], dataBuf[dataPos:])
			controlBuf[g] = ctrl
			dataPos += n
		} else {
			var group [4]uint32
...
func BaseEncodeStreamVByte32GroupSIMD_neon(values []uint32) (ctrl byte, data []byte) {
	if len(values) < 4 {
	...
	inputVec := asm.LoadUint8x16Slice(inputBytes[:16])
	maskVec := asm.LoadUint8x16Slice(streamVByte32EncodeShuffleMasks[ctrl][:16])
	shuffled := inputVec.TableLookupBytes(maskVec)
...
func BaseEncodeStreamVByte32GroupSIMDInto_neon(values []uint32, dst []uint8) (ctrl byte, n int) {
